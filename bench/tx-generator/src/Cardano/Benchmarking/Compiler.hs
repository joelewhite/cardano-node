...
initCollaterals = do
  isAnyPlutusMode >>= \case
    ...
    True -> do
      ...
      emit $ CreateChange LocalSocket (PayToCollateral $ KeyName "pass-partout") (safeCollateral + tx_fee) 1  
...
splittingPhase = do
  ...
  if plutus then createChangeRecursive       minValuePerInput (tx_count * inputs_per_tx)
            else createChangeRecursivePlutus minValuePerInput (tx_count * inputs_per_tx)
 where
